 ...
 *
 * <p>The {@code NetworkServer} class extends {@code NetworkPoint} to implement server-specific
 * behavior for initializing the server, handling incoming connections, and shutting down.</p>
 *
 * @see NetworkPoint
 * @see BossHandler
 */
public class NetworkServer extends NetworkPoint {
	...
	/**
	 * Constructs a new {@code NetworkServer} with the specified cluster node.
	 *
	 * @param clusterNode the {@link AbstractNode} representing the cluster node
	 */
	public NetworkServer(AbstractNode clusterNode) {
		super(clusterNode);
	}
